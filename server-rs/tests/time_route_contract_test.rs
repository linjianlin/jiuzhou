...
use jiuzhou_server_rs::edge::http::routes::time::{
    GameTimeSnapshotView, TimeRouteServices,
};
...
impl TimeRouteServices for FakeTimeServices {
    fn get_game_time_snapshot<'a>(
        &'a self,
    ) -> Pin<Box<dyn Future<Output = Result<Option<GameTimeSnapshotView>, BusinessError>> + Send + 'a>> {
