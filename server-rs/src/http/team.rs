...
pub async fn get_nearby_teams_handler(
    ...
    let current_map_id = if let Some(map_id) = query
        .map_id
        .as_deref()
        .map(str::trim)
        .filter(|v| !v.is_empty())
    {
        ...
        state
            .database
            .fetch_optional(
                "SELECT current_map_id FROM characters WHERE id = $1 LIMIT 1",
                |query| query.bind(character_id),
            )
            .await?
            .and_then(|row| {
    ...
    let leader_ids: Vec<i64> = rows
        .iter()
        .filter_map(|row| {
            row.try_get::<Option<i32>, _>("leader_id")
                .ok()
                .flatten()
                .map(i64::from)
        })
    ...
    let data = rows
        ...
        .map(|row| TeamEntryDto {
            ...
            name: row
                ...
                .unwrap_or_default(),
            leader: row
                .try_get::<Option<String>, _>("leader_name")
                .unwrap_or(None)
                .unwrap_or_default(),
            leader_month_card_active: row
                .try_get::<Option<i32>, _>("leader_id")
                .unwrap_or(None)
                .map(i64::from)
...
pub async fn get_received_invitations_handler(
    ...
    let inviter_ids: Vec<i64> = rows
        .iter()
        .map(|row| opt_i64_from_i32(row, "inviter_id"))
        .filter(|id| *id > 0)
        .collect();
    let month_map = load_month_card_map_by_character_ids(&state, inviter_ids).await?;
    let data = rows
        .into_iter()
        .map(|row| TeamInvitationDto {
            id: row
                .try_get::<Option<String>, _>("id")
                .unwrap_or(None)
                .unwrap_or_default(),
            ...
            goal: row
                ...
                .unwrap_or_default(),
            inviter_name: row
                .try_get::<Option<String>, _>("inviter_name")
                .unwrap_or(None)
                .unwrap_or_default(),
            inviter_month_card_active: month_map
                .get(&opt_i64_from_i32(&row, "inviter_id"))
                .copied()
                .unwrap_or(false),
...
pub async fn create_team_handler(
    ...
    if let Some(debug_realtime) = result
        .data
        .as_ref()
        .and_then(|data| data.debug_realtime.clone())
    {
...
pub async fn leave_team_handler(
    ...
    let result = state
        .database
        .with_transaction(|| async { leave_team_tx(&state, payload.character_id).await })
        .await?;
    if result.success {
        if let Some((team_id, _)) = membership.as_ref() {
            ...
            let debug_realtime = build_team_update_payload(
                source,
                Some(team_id.as_str()),
...
pub async fn invite_to_team_handler(
    ...
    if let Some(debug_realtime) = result.debug_realtime.clone() {
        emit_team_update_to_characters(
            &state,
            &[payload.inviter_id, payload.invitee_id],
            &debug_realtime,
        );
    }
...
pub async fn handle_team_invitation_handler(
    ...
    let result = state
        .database
        .with_transaction(|| async {
            handle_team_invitation_tx(
                &state,
                payload.character_id,
                payload.invitation_id.trim(),
                payload.accept,
            )
            .await
        })
...
pub async fn update_team_settings_handler(
    ...
    if result.success
        && !result
            .message
            .as_deref()
            .is_some_and(|message| message == "无需更新")
    {
        let debug_realtime = build_team_update_payload(
...
async fn load_character_team_membership(
    ...
    Ok(row.map(|row| {
        (
            row.try_get::<Option<String>, _>("team_id")
                .unwrap_or(None)
                .unwrap_or_default(),
            row.try_get::<Option<String>, _>("role")
                .unwrap_or(None)
...
async fn load_character_snapshot(
    ...
    Ok(row.map(|row| {
        (
            row.try_get::<Option<String>, _>("nickname")
                .unwrap_or(None)
                .unwrap_or_default(),
            row.try_get::<Option<String>, _>("current_map_id")
                .unwrap_or(None),
...
fn extract_team_update_payload_from_result_data(
    data: &serde_json::Value,
) -> Option<TeamUpdatePayload> {
...
async fn create_team_tx(
    ...
    if load_character_team_membership(state, character_id)
        .await?
        .is_some()
    {
        return Ok(ServiceResult {
            success: false,
            message: Some("你已在队伍中，请先退出当前队伍".to_string()),
            data: None,
        });
    }
    let Some((nickname, current_map_id)) = load_character_snapshot(state, character_id).await?
    else {
        return Ok(ServiceResult {
            success: false,
    ...
    let team_name = name
        ...
        .unwrap_or_else(|| format!("{}的小队", nickname));
    let team_goal = goal
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| "组队冒险".to_string());
    ...
    Ok(ServiceResult {
        ...
        data: Some(TeamCreateResultDto {
            ...
            name: team_name,
            debug_realtime: Some(build_team_update_payload(
                "create_team",
                Some(team_id.as_str()),
                Some("队伍创建成功"),
            )),
...
async fn leave_team_tx(
    ...
    let Some((team_id, role)) = load_character_team_membership(state, character_id).await? else {
        return Ok(ServiceResult {
            success: false,
            message: Some("你不在任何队伍中".to_string()),
            data: None,
        });
    };
    ...
    if role == "leader" {
        ...
        if let Some(next_member) = next_member {
            ...
            state
                .database
                .execute("UPDATE teams SET leader_id = $1 WHERE id = $2", |query| {
                    query.bind(new_leader_id).bind(&team_id)
                })
                .await?;
            state.database.execute(
            ...
            state
                .database
                .execute("DELETE FROM teams WHERE id = $1", |query| {
                    query.bind(&team_id)
                })
                .await?;
            return Ok(ServiceResult {
                success: true,
                message: Some("队伍已解散（无其他成员）".to_string()),
                data: None,
            });
        }
    }
...
async fn disband_team_tx(
    ...
    if leader_id != character_id {
        return Ok(ServiceResult {
            success: false,
            message: Some("只有队长才能解散队伍".to_string()),
            data: None,
        });
    }
    state
...
async fn invite_to_team_tx(
    ...
    if member_count >= max_members {
        return Ok(TeamInviteResponse {
            success: false,
            message: "队伍已满".to_string(),
            invitation_id: None,
            debug_realtime: None,
        });
    }

    if load_character_team_membership(state, invitee_id)
        .await?
        .is_some()
    {
        return Ok(TeamInviteResponse {
            success: false,
            message: "该玩家已在队伍中".to_string(),
            invitation_id: None,
            debug_realtime: None,
        });
    }
...
async fn get_team_applications_tx(
    ...
    if leader_id != character_id {
        return Ok(ServiceResult {
            success: false,
            message: Some("只有队长才能查看申请".to_string()),
            data: None,
        });
    }
    let rows = state.database.fetch_all(
    ...
    let applicant_ids: Vec<i64> = rows
        .iter()
        .filter_map(|row| {
            row.try_get::<Option<i32>, _>("applicant_id")
                .ok()
                .flatten()
                .map(i64::from)
        })
    ...
    let data = rows
        ...
        .map(|row| TeamApplicationDto {
            ...
            name: row
                ...
                .unwrap_or(None)
                .unwrap_or_default(),
            month_card_active: row
                .try_get::<Option<i32>, _>("applicant_id")
                .unwrap_or(None)
                .map(i64::from)
                .and_then(|id| month_map.get(&id).copied())
                .unwrap_or(false),
...
async fn apply_to_team_tx(
    ...
    if load_character_team_membership(state, character_id)
        ...
        .is_some()
    {
        return Ok(TeamApplyResponse {
            success: false,
            message: "你已在队伍中".to_string(),
            application_id: None,
            auto_joined: None,
            debug_realtime: None,
        });
    ...
    let character_realm = load_character_realm(state, character_id)
        .await?
        .unwrap_or_else(|| "凡人".to_string());
    let join_min_realm = team
        .try_get::<Option<String>, _>("join_min_realm")?
        .unwrap_or_else(|| "凡人".to_string());
    if compare_realm_rank(&character_realm, &join_min_realm) < 0 {
        return Ok(TeamApplyResponse {
            success: false,
            message: format!("境界不足，需要{}以上", join_min_realm),
            application_id: None,
            auto_joined: None,
            debug_realtime: None,
        });
...
async fn handle_team_application_tx(
    ...
    let Some(application) = application else {
        return Ok(ServiceResult {
            success: false,
            message: Some("申请不存在或已处理".to_string()),
            data: None,
        });
    };
    ...
    if leader_id != character_id {
        return Ok(ServiceResult {
            success: false,
            message: Some("只有队长才能处理申请".to_string()),
            data: None,
        });
    }
    let team_id = application
    ...
    if approve {
        ...
        if member_count >= max_members {
            ...
            return Ok(ServiceResult {
                success: false,
                message: Some("队伍已满".to_string()),
                data: None,
            });
        }
        if load_character_team_membership(state, applicant_id)
            .await?
            .is_some()
        {
            state.database.execute(
            ...
            return Ok(ServiceResult {
                success: false,
                message: Some("该玩家已加入其他队伍".to_string()),
                data: None,
            });
        }
        state
...
async fn handle_team_invitation_tx(
    ...
    let Some(invite) = invite else {
        return Ok(ServiceResult {
            success: false,
            message: Some("邀请不存在或已处理".to_string()),
            data: None,
        });
    };
    let team_id = invite
    ...
    if accept {
            ...
            .is_some()
        {
            ...
            return Ok(ServiceResult {
                success: false,
                message: Some("你已在其他队伍中".to_string()),
                data: None,
            });
        }
        let member_count = invite
            .try_get::<Option<i64>, _>("member_count")?
            .unwrap_or_default();
...
async fn kick_team_member_tx(
    ...
    if role != "leader" {
        return Ok(ServiceResult {
            success: false,
            message: Some("只有队长才能踢人".to_string()),
            data: None,
        });
    }
    ...
    if target.is_none() {
        return Ok(ServiceResult {
            success: false,
            message: Some("该玩家不在队伍中".to_string()),
            data: None,
        });
    }
    state
...
async fn transfer_team_leader_tx(
    ...
    let Some((team_id, role)) = load_character_team_membership(state, current_leader_id).await?
    else {
        return Ok(ServiceResult {
            success: false,
            message: Some("你不在任何队伍中".to_string()),
    ...
    if role != "leader" {
        return Ok(ServiceResult {
            success: false,
            message: Some("只有队长才能转让".to_string()),
            data: None,
        });
    }
    let next_leader = state
        .database
        .fetch_optional(
            "SELECT id FROM team_members WHERE team_id = $1 AND character_id = $2 LIMIT 1",
            |query| query.bind(&team_id).bind(new_leader_id),
...
async fn update_team_settings_tx(
    ...
    if leader_id != character_id {
        return Ok(ServiceResult {
            success: false,
            message: Some("只有队长才能修改设置".to_string()),
            data: None,
        });
    }
    ...
    if let Some(goal) = settings.goal.map(|value| value.trim().to_string()) {
        updates.push(match updates.len() {
            0 => "goal = $1",
            1 => "goal = $2",
            2 => "goal = $3",
            3 => "goal = $4",
    ...
    if let Some(join_min_realm) = settings
        .join_min_realm
    ...
    if let Some(auto_join_enabled) = settings.auto_join_enabled {
        updates.push(match updates.len() {
            0 => "auto_join_enabled = $1",
            1 => "auto_join_enabled = $2",
            2 => "auto_join_enabled = $3",
            3 => "auto_join_enabled = $4",
            4 => "auto_join_enabled = $5",
            _ => "auto_join_enabled = $6",
        });
        bind_auto_join_enabled = Some(auto_join_enabled);
    }
    if let Some(auto_join_min_realm) = settings
        .auto_join_min_realm
        .map(|value| value.trim().to_string())
    {
        updates.push(match updates.len() {
            0 => "auto_join_min_realm = $1",
            1 => "auto_join_min_realm = $2",
            2 => "auto_join_min_realm = $3",
            3 => "auto_join_min_realm = $4",
            4 => "auto_join_min_realm = $5",
            _ => "auto_join_min_realm = $6",
        });
    ...
    if updates.is_empty() {
        return Ok(ServiceResult {
            success: true,
            message: Some("无需更新".to_string()),
            data: None,
        });
    }

    let sql = format!(
        "UPDATE teams SET {}, updated_at = NOW() WHERE id = ${}",
        updates.join(", "),
        updates.len() + 1
    );
    state
        .database
        .execute(&sql, move |query| {
            let query = if let Some(value) = bind_name.clone() {
                query.bind(value)
            } else {
                query
            };
            let query = if let Some(value) = bind_goal.clone() {
                query.bind(value)
            } else {
                query
            };
            let query = if let Some(value) = bind_join_min_realm.clone() {
                query.bind(value)
            } else {
                query
            };
            let query = if let Some(value) = bind_auto_join_enabled {
                query.bind(value)
            } else {
                query
            };
            let query = if let Some(value) = bind_auto_join_min_realm.clone() {
                query.bind(value)
            } else {
                query
            };
            let query = if let Some(value) = bind_is_public {
                query.bind(value)
            } else {
                query
            };
...
async fn load_team_info(state: &AppState, team_id: &str) -> Result<Option<TeamInfoDto>, AppError> {
    ...
    Ok(Some(TeamInfoDto {
        ...
        join_min_realm: row
            .try_get::<Option<String>, _>("join_min_realm")?
            .unwrap_or_else(|| "凡人".to_string()),
        auto_join_enabled: row
            .try_get::<Option<bool>, _>("auto_join_enabled")?
            .unwrap_or(false),
        auto_join_min_realm: row
            .try_get::<Option<String>, _>("auto_join_min_realm")?
            .unwrap_or_else(|| "凡人".to_string()),
...
async fn load_team_members(
    ...
    let character_ids: Vec<i64> = rows
        .iter()
        .filter_map(|row| {
            row.try_get::<Option<i32>, _>("character_id")
                .ok()
        ...
        .map(|row| {
            ...
            TeamMemberDto {
                ...
                role: row
                    ...
                    .unwrap_or(None)
                    .unwrap_or_else(|| "member".to_string()),
                realm: if sub_realm.is_empty() || realm == "凡人" {
                    realm
                } else {
