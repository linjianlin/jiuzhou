...
pub async fn load_wander_overview_data(
    ...
    let latest_finished_story = load_latest_finished_story_with_episodes(&state, character_id).await?;
    ...
    let cooldown_state = build_wander_cooldown_state(latest_episode.as_ref().map(|row| row.created_at.as_str()), now, state.config.service.node_env.as_str());
    let current_episode = if let Some(episode) = latest_episode.clone() {
        if episode.chosen_option_index.is_none() || episode.chosen_at.is_none() || cooldown_state.is_cooling_down {
    ...
    let current_generation_job = latest_generation_job.as_ref().and_then(|row| {
        ...
        let expose = pending_or_failed
            && ((current_episode.is_some() && row.generated_episode_id.as_deref() == current_episode.as_ref().map(|ep| ep.id.as_str()) && current_episode.as_ref().and_then(|ep| ep.chosen_at.as_deref()).is_none())
    ...
    let has_pending_episode = current_episode.as_ref().map(|ep| ep.chosen_option_index.is_none()).unwrap_or(false);
    ...
    let can_generate = ai_available
        ...
        && current_generation_job.as_ref().map(|job| job.status.as_str()) != Some("pending");
    ...
    if result.success {
        if let Some(job) = result.data.as_ref().map(|data| data.job.clone()) {
            ...
            state.database.after_transaction_commit(async move {
                jobs::enqueue_wander_generation_job(state_for_enqueue, character_id, job.generation_id).await
            }).await?;
        }
        if let Ok(overview) = load_wander_overview_data(&state, actor.character_id).await {
            emit_wander_update_to_user(&state, actor.user_id, &build_wander_update_payload(overview));
        }
    }
...
pub async fn choose_wander_episode_option(
    ...
    let state_for_enqueue = state.clone();
    let result = state.database.with_transaction(|| async {
        choose_wander_episode_option_tx(&state, actor.character_id, &payload.episode_id, payload.option_index).await
    }).await?;
    if result.success {
        if let Some(job) = result.data.as_ref().map(|data| data.job.clone()) {
            let character_id = actor.character_id;
...
fn opt_i64_from_i32_opt(row: &sqlx::postgres::PgRow, column: &str) -> Result<Option<i64>, AppError> {
    Ok(row.try_get::<Option<i32>, _>(column)?.map(i64::from))
}

fn resolve_wander_story_max_episode_index(story_seed: i32, persisted_episode_count: Option<i64>) -> i64 {
...
async fn load_latest_episode_row(state: &AppState, character_id: i64) -> Result<Option<WanderEpisodeRowData>, AppError> {
...
async fn load_story_with_episodes(state: &AppState, character_id: i64, status: &str) -> Result<Option<WanderStoryDto>, AppError> {
    ...
    let Some(story) = story else { return Ok(None); };
    let story_id = story.try_get::<Option<String>, _>("id")?.unwrap_or_default();
    let episodes = load_story_episodes(state, &story_id).await?;
    Ok(Some(WanderStoryDto {
        id: story_id,
        status: story.try_get::<Option<String>, _>("status")?.unwrap_or_else(|| status.to_string()),
        theme: story.try_get::<Option<String>, _>("story_theme")?.unwrap_or_default(),
        premise: story.try_get::<Option<String>, _>("story_premise")?.unwrap_or_default(),
        summary: story.try_get::<Option<String>, _>("story_summary")?.unwrap_or_default(),
        ...
        created_at: story.try_get::<Option<String>, _>("created_at_text")?.unwrap_or_default(),
        updated_at: story.try_get::<Option<String>, _>("updated_at_text")?.unwrap_or_default(),
        episodes,
    }))
}

async fn load_story_with_episodes_by_id(state: &AppState, story_id: &str) -> Result<Option<WanderStoryDto>, AppError> {
    let story = state.database.fetch_optional(
    ...
    Ok(Some(WanderStoryDto {
        id: story_id,
        status: story.try_get::<Option<String>, _>("status")?.unwrap_or_else(|| "active".to_string()),
        theme: story.try_get::<Option<String>, _>("story_theme")?.unwrap_or_default(),
        premise: story.try_get::<Option<String>, _>("story_premise")?.unwrap_or_default(),
        summary: story.try_get::<Option<String>, _>("story_summary")?.unwrap_or_default(),
        episode_count: opt_i64_from_i32(&story, "episode_count"),
...
async fn load_latest_finished_story_with_episodes(state: &AppState, character_id: i64) -> Result<Option<WanderStoryDto>, AppError> {
    let story = state.database.fetch_optional(
    ...
    Ok(Some(WanderStoryDto {
        id: story_id,
        status: story.try_get::<Option<String>, _>("status")?.unwrap_or_else(|| "finished".to_string()),
        theme: story.try_get::<Option<String>, _>("story_theme")?.unwrap_or_default(),
        premise: story.try_get::<Option<String>, _>("story_premise")?.unwrap_or_default(),
        summary: story.try_get::<Option<String>, _>("story_summary")?.unwrap_or_default(),
        episode_count: opt_i64_from_i32(&story, "episode_count"),
...
async fn load_story_episodes(state: &AppState, story_id: &str) -> Result<Vec<WanderEpisodeDto>, AppError> {
    ...
    rows.into_iter().map(parse_episode_row).map(|row| row.map(map_episode_row)).collect()
}

async fn load_generated_titles(state: &AppState, character_id: i64) -> Result<Vec<WanderGeneratedTitleDto>, AppError> {
    let rows = state.database.fetch_all(
    ...
    Ok(rows.into_iter().map(|row| WanderGeneratedTitleDto {
        ...
        color: normalize_wander_title_color(row.try_get::<Option<String>, _>("color").unwrap_or(None)),
        effects: normalize_wander_title_effects(row.try_get::<Option<serde_json::Value>, _>("effects").unwrap_or(None)),
        is_equipped: row.try_get::<Option<bool>, _>("is_equipped").unwrap_or(None).unwrap_or(false),
        obtained_at: row.try_get::<Option<String>, _>("obtained_at_text").unwrap_or(None).unwrap_or_default(),
    }).collect())
}

async fn load_latest_generation_job(state: &AppState, character_id: i64) -> Result<Option<WanderGenerationJobRowData>, AppError> {
    let row = state.database.fetch_optional(
    ...
    Ok(row.map(|row| WanderGenerationJobRowData {
        generation_id: row.try_get::<Option<String>, _>("id").unwrap_or(None).unwrap_or_default(),
        ...
        generated_episode_id: row.try_get::<Option<String>, _>("generated_episode_id").unwrap_or(None),
    }).map(|row| row))
}
...
async fn load_generation_job_by_id_for_update(
    ...
    Ok(row.map(|row| WanderGenerationJobWithDayKeyRowData {
        day_key: normalize_date_key(row.try_get::<Option<String>, _>("day_key_text").unwrap_or(None).unwrap_or_default()),
        status: row.try_get::<Option<String>, _>("status").unwrap_or(None).unwrap_or_else(|| "pending".to_string()),
        error_message: row.try_get::<Option<String>, _>("error_message").unwrap_or(None),
        generated_episode_id: row.try_get::<Option<String>, _>("generated_episode_id").unwrap_or(None),
    }))
...
async fn load_character_exists(state: &AppState, character_id: i64) -> Result<bool, AppError> {
    Ok(state.database.fetch_optional(
...
async fn create_wander_generation_job_tx(
    ...
    if latest_episode.as_ref().and_then(|episode| episode.chosen_option_index).is_none() && latest_episode.is_some() {
    ...
    if let Some(job) = latest_generation_job.clone() {
        if job.status == "pending" {
            return Ok(ServiceResult {
                ...
                data: Some(WanderGenerateQueueResultDto { job: map_generation_job_row(job) }),
            });
        }
    }
    ...
    let generation_day_key = resolve_wander_generation_day_key(
        latest_episode.as_ref().map(|episode| episode.day_key.as_str()),
        now,
    );
...
async fn choose_wander_episode_option_tx(
    ...
    let Some(mut episode) = load_episode_row_by_id_for_update(state, character_id, episode_id).await? else {
        return Ok(failure_result("奇遇幕次不存在"));
    };

    let chosen_option_text = if episode.chosen_option_index.is_none() {
        let Some(option_text) = episode.option_texts.get(normalized_option_index as usize).cloned() else {
    ...
    if let Some(job) = load_latest_generation_job_by_episode(state, character_id, &episode.id).await? {
        if job.status == "pending" {
            ...
            return Ok(ServiceResult {
                ...
                data: Some(WanderChooseResultDto { story, job: map_generation_job_row(job) }),
            });
        }
    }
...
async fn process_pending_generation_job_tx(
    ...
    let Some(job) = load_generation_job_by_id_for_update(state, character_id, generation_id).await? else {
    ...
    if job.status != "pending" {
        return Ok(ServiceResult {
            ...
            data: Some(WanderGenerationProcessResult {
                status: if job.status == "generated" { "generated".to_string() } else { "failed".to_string() },
    ...
    if let Some(target_episode_id) = job.generated_episode_id.clone() {
        let Some(target_episode) = load_episode_row_by_id(state, character_id, &target_episode_id).await? else {
        ...
        if target_episode.chosen_option_index.is_none() || target_episode.chosen_option_text.is_none() {
    ...
    if let Some(existing_episode) = load_episode_row_by_day_key(state, character_id, &job.day_key).await? {
...
fn normalize_wander_resolution_outcome(
    ...
    WanderResolutionOutcome {
        ...
        reward_title_effects: normalize_wander_title_effects_value_map(outcome.reward_title_effects),
    }
}
...
const WANDER_OPTION_EXAMPLE_JSON: &str = r#"["先借檐避雨，再试探来意","绕到桥下暗查灵息","收敛气机，静观其变"]"#;
...
const WANDER_STORY_PREMISE_EXAMPLE: &str = "你循着残留血迹误入谷口深处，才觉今夜盘踞此地的异物并非寻常山兽。";
...
async fn settle_wander_episode_choice(
    ...
    let chosen_option_index = episode.chosen_option_index.ok_or_else(|| AppError::config("云游结算缺少已确认的选项"))?;
    let chosen_option_text = episode.chosen_option_text.as_deref().ok_or_else(|| AppError::config("云游结算缺少已确认的选项"))?;
    let story = state.database.fetch_optional(
    ...
    let story_theme = story.try_get::<Option<String>, _>("story_theme")?.unwrap_or_else(|| "云游奇遇".to_string());
    let story_premise = story.try_get::<Option<String>, _>("story_premise")?.unwrap_or_default();
    let story_summary = story.try_get::<Option<String>, _>("story_summary")?.unwrap_or_default();
    ...
    let nickname = character_row.try_get::<Option<String>, _>("nickname")?.unwrap_or_else(|| "道友".to_string());
    ...
    let realm_text = if realm.trim() == "凡人" || sub_realm.as_deref().unwrap_or_default().trim().is_empty() {
        realm.trim().to_string()
    } else {
        format!("{}·{}", realm.trim(), sub_realm.as_deref().unwrap_or_default().trim())
    };
    let story_seed = story.try_get::<Option<i32>, _>("story_seed")?.unwrap_or(1).max(1);
    ...
    let previous_episodes = load_wander_previous_episode_context(state, &episode.story_id, &episode.id, story_location.full_name.as_str()).await?;
    ...
    let resolution_repair_system_message = build_wander_ai_resolution_repair_system_message(episode.is_ending);
    ...
    let outcome = normalize_wander_resolution_outcome(WanderResolutionOutcome {
        summary,
        ending_type,
        reward_title_name: if reward_title_name.trim().is_empty() { None } else { Some(reward_title_name) },
        reward_title_desc: if reward_title_desc.trim().is_empty() { None } else { Some(reward_title_desc) },
        reward_title_color: if reward_title_color.trim().is_empty() { None } else { Some(reward_title_color) },
        reward_title_effects,
    });
    ...
    let reward_title_id = if episode.is_ending {
        ...
        let reward_title_name = outcome.reward_title_name.as_deref().expect("ending reward title name checked");
        let reward_title_desc = outcome.reward_title_desc.as_deref().expect("ending reward title desc checked");
    ...
    if let Some(existing_episode) = load_episode_row_by_day_key(state, character_id, &next_day_key).await? {
...
fn build_wander_ai_resolution_user_message(
    ...
    let resolution_mode = if is_ending { "must_end" } else { "must_continue" };
    serde_json::json!({
...
fn parse_wander_story_partner_context(
    value: serde_json::Value,
) -> Option<WanderAiStoryPartnerDto> {
        ...
        .filter_map(|map| {
            ...
            let rooms = map
                ...
                .filter_map(|room| {
                    ...
                    (!room.id.trim().is_empty() && !area_name.trim().is_empty()).then_some((room.id, area_name))
                })
                .collect::<Vec<_>>();
            (!map.id.trim().is_empty() && !rooms.is_empty()).then_some((map.id, map_name, region, rooms))
        })
        .collect::<Vec<_>>();
    maps.sort_by(|left, right| left.0.cmp(&right.0));
    let Some((map_id, map_name, region, rooms)) = maps.get(deterministic_story_other_player_index(story_seed, maps.len())).cloned() else {
...
async fn load_wander_story_partner_context(
    ...
    Ok(Some(WanderAiStoryPartnerDto {
        ...
        nickname: row.try_get::<Option<String>, _>("nickname")?.unwrap_or_else(|| "伙伴".to_string()),
        name: row.try_get::<Option<String>, _>("name")?.unwrap_or_else(|| "伙伴".to_string()),
        description: row.try_get::<Option<String>, _>("description")?,
        role: row.try_get::<Option<String>, _>("role")?.unwrap_or_else(|| "伙伴".to_string()),
        quality: row.try_get::<Option<String>, _>("quality")?.unwrap_or_else(|| "黄".to_string()),
    }))
}
...
async fn load_wander_previous_episode_context(
    ...
    Ok(rows
        ...
        .map(|row| WanderAiPreviousEpisodeContextDto {
            ...
            title: row.try_get::<Option<String>, _>("episode_title").unwrap_or(None).unwrap_or_default(),
            opening: row.try_get::<Option<String>, _>("opening").unwrap_or(None).unwrap_or_default(),
            chosen_option_text: row.try_get::<Option<String>, _>("chosen_option_text").unwrap_or(None).unwrap_or_default(),
            summary: row.try_get::<Option<String>, _>("episode_summary").unwrap_or(None).unwrap_or_default(),
            is_ending: row.try_get::<Option<bool>, _>("is_ending").unwrap_or(None).unwrap_or(false),
        })
        .collect())
}
...
async fn create_wander_episode_for_day_key(
    ...
    let character = state.database.fetch_optional(
    ...
    let nickname = character.try_get::<Option<String>, _>("nickname")?.unwrap_or_else(|| "道友".to_string());
    let realm = character.try_get::<Option<String>, _>("realm")?.unwrap_or_else(|| "凡人".to_string());
    ...
    let current_story_location = if let Some(active_story) = active_story.as_ref() {
        resolve_wander_story_location_context_from_seed(
            active_story.try_get::<Option<i32>, _>("story_seed")?.unwrap_or(1).max(1),
        )?
    } else {
        resolve_wander_story_location_context_from_seed(new_story_seed.unwrap_or(1))?
    };
    let (story_id, _story_seed, next_episode_index, max_episode_index, theme, premise, story_summary, story_partner, story_other_player) = if let Some(active_story) = active_story {
        let story_id = active_story.try_get::<Option<String>, _>("id")?.unwrap_or_default();
        let current_episode_count = opt_i64_from_i32(&active_story, "episode_count").max(0);
        let story_seed = active_story.try_get::<Option<i32>, _>("story_seed")?.unwrap_or(1).max(1);
        let theme = active_story.try_get::<Option<String>, _>("story_theme")?.unwrap_or_else(|| "云游奇遇".to_string());
        let premise = active_story.try_get::<Option<String>, _>("story_premise")?.unwrap_or_else(|| format!("{}行至{}，感应到一缕异样机缘。", nickname, realm_text));
        let story_summary = active_story.try_get::<Option<String>, _>("story_summary")?.unwrap_or_default();
        ...
        (story_id, story_seed, current_episode_count + 1, resolve_wander_target_episode_count(story_seed), theme, premise, story_summary, story_partner, story_other_player)
    } else {
        let story_id = build_story_id();
        let theme = "云游奇遇".to_string();
        let premise = format!("{}以{}之身踏上云游，偶遇一缕未明机缘。", nickname, realm_text);
        ...
        let story_other_player_snapshot = if should_include_wander_story_other_player(story_seed) {
            load_wander_story_other_player_snapshot_for_new_story(state, character_id, story_seed).await?
        } else {
            None
        };
        let story_partner = story_partner_snapshot.clone().and_then(parse_wander_story_partner_context);
        let story_other_player = story_other_player_snapshot.clone().and_then(parse_wander_story_other_player_context);
        state.database.execute(
        ...
        (story_id, story_seed, 1, resolve_wander_target_episode_count(story_seed), theme, premise, String::new(), story_partner, story_other_player)
    };

    let has_team = load_wander_has_team_context(state, character_id).await?;
    let previous_episodes = load_wander_previous_episode_context_for_story(state, &story_id, current_story_location.full_name.as_str()).await?;
    ...
    let setup_user_message = build_wander_ai_setup_user_message(
        ...
        if theme.trim().is_empty() { None } else { Some(theme.as_str()) },
        if premise.trim().is_empty() { None } else { Some(premise.as_str()) },
        if story_summary.trim().is_empty() { None } else { Some(story_summary.as_str()) },
        next_episode_index,
        max_episode_index,
        is_ending,
        &previous_episodes,
    ...
    let persisted_premise = if premise.trim().is_empty() { story_premise } else { premise };
...
fn build_wander_ai_setup_user_message(
    ...
    let resolved_story_summary = resolve_wander_setup_story_summary(story_summary, previous_episodes);
    serde_json::json!({
...
fn should_include_wander_story_partner(story_seed: i32) -> bool {
    deterministic_story_seed_unit_float("wander-story-partner", story_seed) < WANDER_STORY_PARTNER_INCLUDE_RATE
}

fn should_include_wander_story_other_player(story_seed: i32) -> bool {
    deterministic_story_seed_unit_float("wander-story-other-player", story_seed) < WANDER_STORY_OTHER_PLAYER_INCLUDE_RATE
}
...
fn parse_episode_row(row: sqlx::postgres::PgRow) -> Result<WanderEpisodeRowData, AppError> {
    Ok(WanderEpisodeRowData {
        ...
        story_id: row.try_get::<Option<String>, _>("story_id")?.unwrap_or_default(),
        day_key: normalize_date_key(row.try_get::<Option<String>, _>("day_key_text")?.unwrap_or_default()),
        day_index: opt_i64_from_i32(&row, "day_index"),
        episode_title: row.try_get::<Option<String>, _>("episode_title")?.unwrap_or_default(),
        opening: row.try_get::<Option<String>, _>("opening")?.unwrap_or_default(),
        option_texts: row.try_get::<Option<serde_json::Value>, _>("option_texts")?.and_then(|v| v.as_array().cloned()).unwrap_or_default().into_iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect(),
        ...
        episode_summary: row.try_get::<Option<String>, _>("episode_summary")?.unwrap_or_default(),
        is_ending: row.try_get::<Option<bool>, _>("is_ending")?.unwrap_or(false),
        ending_type: row.try_get::<Option<String>, _>("ending_type")?.unwrap_or_else(|| "none".to_string()),
        ...
        reward_title_effects: parse_number_map(row.try_get::<Option<serde_json::Value>, _>("reward_title_effects")?),
...
fn map_episode_row(row: WanderEpisodeRowData) -> WanderEpisodeDto {
    WanderEpisodeDto {
        ...
        opening: row.opening,
        options: row.option_texts.into_iter().enumerate().map(|(index, text)| WanderEpisodeOptionDto { index: index as i64, text }).collect(),
...
fn parse_number_map(value: Option<serde_json::Value>) -> std::collections::BTreeMap<String, f64> {
    value.and_then(|v| v.as_object().cloned()).unwrap_or_default().into_iter().filter_map(|(k,v)| v.as_f64().or_else(|| v.as_i64().map(|n| n as f64)).map(|n| (k,n))).collect()
}

fn normalize_wander_title_color(color: Option<String>) -> Option<String> {
    let normalized = color.as_deref().map(str::trim).filter(|value| !value.is_empty())?;
...
fn normalize_wander_title_effects(value: Option<serde_json::Value>) -> std::collections::BTreeMap<String, f64> {
...
fn normalize_wander_title_effects_value_map(
    ...
    raw.into_iter()
        .filter(|(key, value)| WANDER_TITLE_EFFECT_KEYS.contains(&key.as_str()) && value.is_finite() && *value != 0.0)
        .map(|(key, value)| {
            let normalized = if WANDER_TITLE_RATIO_EFFECT_KEYS.contains(&key.as_str()) {
                (value * WANDER_TITLE_RATIO_EFFECT_PRECISION).round() / WANDER_TITLE_RATIO_EFFECT_PRECISION
            } else {
                value.floor()
            };
            (key, normalized)
...
fn build_wander_cooldown_state(latest_episode_created_at: Option<&str>, now: time::OffsetDateTime, node_env: &str) -> WanderCooldownState {
    let bypass = node_env == "development";
    if bypass {
        return WanderCooldownState { cooldown_until: None, cooldown_remaining_seconds: 0, is_cooling_down: false };
    }
    ...
    let remaining_ms = (cooldown_until.unix_timestamp_nanos() - now.unix_timestamp_nanos()).max(0) / 1_000_000;
    let remaining = ((remaining_ms + 999) / 1_000) as i64;
    WanderCooldownState {
        cooldown_until: Some(cooldown_until.format(&time::format_description::well_known::Rfc3339).unwrap_or_default()),
...
mod tests {
    use super::{
        WanderOverviewDto, resolve_wander_generation_day_key, build_date_key,
        build_wander_cooldown_state, current_local_time, format_wander_cooldown_remaining,
        process_failure_result, process_generated_result,
    };
    ...
    fn generation_day_key_moves_forward_when_latest_is_today() {
        ...
        .expect("timestamp should parse");
        assert_eq!(resolve_wander_generation_day_key(Some("2026-04-12"), now), "2026-04-13");
    }
    ...
    fn pending_generation_process_success_payload_matches_contract() {
        let payload = serde_json::to_value(process_generated_result("episode-1")).expect("result should serialize");
    ...
    fn wander_resolution_user_message_contains_story_context() {
        ...
        let payload: serde_json::Value = serde_json::from_str(&message).expect("message should be json");
        ...
        assert_eq!(payload["player"]["storyPartner"]["description"], "与你结伴而行的灵伴");
        ...
        assert_eq!(payload["storyLocation"]["fullName"], "青云洲·青云郊外·林间空地");
        assert_eq!(payload["story"]["activeTheme"], "云梦夜航");
        assert_eq!(payload["story"]["activePremise"], "你在夜色中踏入云水深处。");
        assert_eq!(payload["story"]["storySummary"], "前两幕里你已追到桥头灯影背后的因果。");
        ...
        assert_eq!(payload["story"]["currentEpisodeOpening"], "夜雨压桥，河雾顺着石栏缓缓爬起。");
        ...
        assert_eq!(payload["outputRules"]["rewardTitleEffectValueMaxMap"]["max_qixue"], 240);
        assert_eq!(payload["outputRules"]["rewardTitleEffectValueMaxMap"]["wugong"], 60);
        assert_eq!(payload["outputRules"]["nonEndingTitleFieldExample"]["endingType"], "none");
        assert_eq!(payload["story"]["previousEpisodes"][0]["title"], "桥头灯影");
        assert_eq!(payload["story"]["previousEpisodes"][0]["locationName"], "青云洲·青云郊外·林间空地");
        assert_eq!(payload["story"]["previousEpisodes"][0]["chosenOptionText"], "先借檐避雨，再试探来意");
        assert_eq!(payload["story"]["previousEpisodes"][0]["summary"], "你先借檐避雨，再试探桥头提灯客的真实来意。");
    ...
    fn wander_resolution_repair_user_message_contains_original_task_and_reason() {
        ...
        let repair = super::build_wander_ai_resolution_repair_user_message(original.as_str(), "{bad json}", "rewardTitleColor 非法", true);
        let payload: serde_json::Value = serde_json::from_str(&repair).expect("repair payload should be json");
        assert_eq!(payload["validationReason"], "rewardTitleColor 非法");
        assert_eq!(payload["previousOutput"], "{bad json}");
        assert_eq!(payload["outputRules"]["rewardTitleColorPattern"], "#RRGGBB");
        assert_eq!(payload["originalTask"]["story"]["currentEpisodeTitle"], "云梦终幕");
    ...
    fn wander_story_max_episode_index_prefers_persisted_story_count() {
        ...
        assert_eq!(fallback_zero, super::resolve_wander_target_episode_count(42));
        assert_eq!(fallback_none, super::resolve_wander_target_episode_count(42));
        println!("WANDER_MAX_EPISODE_INDEX={{\"persisted\":{},\"fallbackZero\":{},\"fallbackNone\":{}}}", persisted, fallback_zero, fallback_none);
    }
    ...
    fn wander_new_story_seed_is_positive_and_stable_for_same_timestamp() {
        ...
        assert!(first > 0);
        assert!(next > 0);
        println!("WANDER_NEW_STORY_SEED={{\"first\":{},\"second\":{},\"next\":{}}}", first, second, next);
    }
    ...
    fn wander_setup_story_summary_omits_summary_when_previous_episodes_exist() {
        ...
        println!("WANDER_SETUP_STORY_SUMMARY={{\"withPreviousEpisodes\":null,\"initialStory\":{:?}}}", initial_story);
    }

    #[test]
    fn wander_resolution_prefers_story_partner_snapshot_only() {
        let parsed = super::resolve_wander_story_partner_snapshot_for_resolution(Some(serde_json::json!({
            "partnerId": 9,
        ...
        println!("WANDER_RESOLUTION_PARTNER_SNAPSHOT_ONLY={{\"parsedPartnerId\":{},\"missingIsNone\":{}}}", parsed.as_ref().map(|value| value.partner_id).unwrap_or_default(), missing.is_none());
    }
    ...
    fn wander_setup_user_message_contains_story_level_context() {
        ...
        assert_eq!(payload["player"]["storyOtherPlayer"]["subRealm"], "养气期");
        assert_eq!(payload["storyLocation"]["fullName"], "青云洲·青云郊外·林间空地");
        assert!(payload["promptNoiseHash"].as_str().is_some_and(|value| value.len() == 16));
        assert_eq!(payload["story"]["activeTheme"], "云梦夜航");
        ...
        assert_eq!(payload["outputRules"]["storyPremiseExample"], "你循着残留血迹误入谷口深处，才觉今夜盘踞此地的异物并非寻常山兽。");
        assert_eq!(payload["outputRules"]["optionExample"][0], "先借檐避雨，再试探来意");
    ...
    fn wander_prompt_noise_hash_is_stable_for_same_scope_and_seed() {
        ...
        println!("WANDER_PROMPT_NOISE_HASH={{\"setup\":{:?},\"resolution\":{:?}}}", setup, resolution);
    }
    ...
    fn wander_setup_repair_user_message_contains_original_task_and_reason() {
        ...
        let repair = super::build_wander_ai_setup_repair_user_message(original.as_str(), "{bad json}", "storyTheme 长度必须在 2 到 24 之间", false);
        let payload: serde_json::Value = serde_json::from_str(&repair).expect("repair payload should be json");
        assert_eq!(payload["validationReason"], "storyTheme 长度必须在 2 到 24 之间");
        assert_eq!(payload["previousOutput"], "{bad json}");
        assert_eq!(payload["outputRules"]["optionCount"], 3);
    ...
    fn wander_story_location_from_seed_is_stable_and_non_empty() {
        ...
        println!("WANDER_STORY_LOCATION={}", serde_json::json!({
            "mapId": first.map_id,
    ...
    fn wander_title_read_normalization_filters_invalid_color_and_effects() {
        ...
        assert_eq!(super::normalize_wander_title_color(Some("#4CAF50".to_string())), Some("#4CAF50".to_string()));
        assert_eq!(super::normalize_wander_title_color(Some("green".to_string())), None);
        ...
        println!("WANDER_TITLE_EFFECTS_NORMALIZED={}", serde_json::json!(effects));
    }
    ...
    fn wander_resolution_outcome_is_normalized_before_persist() {
        ...
        assert_eq!(outcome.reward_title_desc.as_deref(), Some("断桥一战后，余威仍镇河潮。"));
        ...
        println!("WANDER_RESOLUTION_OUTCOME_NORMALIZED={}", serde_json::json!({
    ...
    fn wander_ending_reward_validation_rejects_missing_color_or_effects_after_normalization() {
        ...
        let error = super::validate_wander_ending_reward_fields(&outcome).expect_err("missing ending reward fields should fail");
    ...
    fn story_partner_snapshot_parser_accepts_persisted_shape() {
        ...
        println!("WANDER_STORY_PARTNER_SNAPSHOT={}", serde_json::json!({
    ...
    fn pending_generation_process_failure_payload_matches_contract() {
        let payload = serde_json::to_value(process_failure_result("云游奇遇生成能力尚未迁移完成")).expect("result should serialize");
        assert_eq!(payload["data"]["status"], "failed");
        assert_eq!(payload["data"]["errorMessage"], "云游奇遇生成能力尚未迁移完成");
