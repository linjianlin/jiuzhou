...
pub async fn get_overview(
    ...
    let breakthrough = next_realm.as_ref().and_then(|_| {
        config
            .breakthroughs
            .iter()
            .find(|entry| entry.from.trim() == current_realm)
            .cloned()
    });
    ...
    let rewards = build_rewards_view(
        breakthrough
            .as_ref()
            .and_then(|entry| entry.rewards.clone()),
    );
...
async fn breakthrough_to_next_realm(
    ...
    for item in &cost_items {
        let have =
            get_item_qty_in_bag(state, preview_state.character_id, &item.item_def_id).await?;
        if have < item.qty {
...
fn filter_volatile_requirements(
    requirements: Vec<BreakthroughRequirement>,
) -> Vec<BreakthroughRequirement> {
    requirements
        .into_iter()
        .filter(|requirement| {
            matches!(
                requirement.requirement_type.as_str(),
                "main_technique_layer_min" | "main_and_sub_technique_layer_min"
            )
        })
        .collect()
}

fn build_requirement_failure_result(
    requirement: &RealmRequirementView,
) -> ServiceResult<RealmBreakthroughData> {
...
fn build_realm_overview_dto(
    ...
    let can_breakthrough = next_realm.is_some()
        ...
        && costs
            .iter()
            .all(|item| item.status.as_deref() != Some("todo"));
        ...
        match requirement.requirement_type.as_str() {
            "exp_min" => {
                ...
                out.push(RealmRequirementView {
                    ...
                    title,
                    detail: format!("经验 ≥ {}（当前 {}）", min.to_string(), exp.to_string()),
                    status: if exp >= min { "done" } else { "todo" }.to_string(),
                    source_type: None,
                    source_ref: None,
                });
            }
            "spirit_stones_min" => {
                ...
                out.push(RealmRequirementView {
                    ...
                    status: if spirit_stones >= min { "done" } else { "todo" }.to_string(),
                    source_type: None,
                    source_ref: None,
                });
            }
            "main_technique_layer_min" => {
                ...
                if let Some((name, layer)) = &main_tech {
                    out.push(RealmRequirementView {
                        id: if id.is_empty() {
                            format!("maintech-{min_layer}")
                        } else {
                            id
                        },
                        title,
                        detail: format!("{}（主功法）≥ {} 层（当前 {}）", name, min_layer, layer),
                        status: if *layer >= min_layer { "done" } else { "todo" }.to_string(),
                        source_type: None,
                        source_ref: None,
                    });
                } else {
                    out.push(RealmRequirementView {
                        ...
                        title,
                        detail: format!("未装备主功法（需要 ≥ {} 层）", min_layer),
            ...
            "technique_layer_min" => {
                ...
                let min_layer = requirement.min_layer.unwrap_or_default().max(0);
                let layer = if technique_id.trim().is_empty() {
                    0
                } else {
                    get_technique_layer(state, character_id, &technique_id).await?
                };
                let tech_name = technique_map
                    .get(&technique_id)
                    .cloned()
                    .unwrap_or_else(|| {
                ...
                out.push(RealmRequirementView {
                    id: if id.is_empty() {
                        format!("{}-{}", technique_id, min_layer)
                    } else {
                        id
                    },
                    title,
                    detail: format!("{} ≥ {} 层（当前 {}）", tech_name, min_layer, layer),
                    status: if layer >= min_layer { "done" } else { "todo" }.to_string(),
                    source_type: None,
                    source_ref: None,
                });
            ...
            "main_and_sub_technique_layer_min" => {
                let min_layer = requirement.min_layer.unwrap_or_default().max(0);
                let equipped_subs = if let Some(cache) = &equipped_subs_cache {
                    cache.clone()
                } else {
                    let loaded = get_equipped_sub_techniques(state, character_id).await?;
                    equipped_subs_cache = Some(loaded.clone());
                    loaded
                };
                if let Some((name, layer)) = &main_tech {
                    let best_sub = equipped_subs.iter().max_by_key(|(_, layer, _)| *layer);
                    let ok_sub = equipped_subs
                        .iter()
                        .any(|(_, layer, _)| *layer >= min_layer);
                    let sub_text = if let Some((sub_name, sub_layer, slot)) = best_sub {
                        format!(
                            "{}（副{} 当前 {}） ≥{}",
                            sub_name, slot, sub_layer, min_layer
                        )
                    } else {
                        format!("未装备副功法 ≥{}", min_layer)
                    };
                    out.push(RealmRequirementView {
                        id: if id.is_empty() {
                            format!("main-sub-{min_layer}")
                        } else {
                            id
                        },
                        title,
                        detail: format!(
                            "{}（主 当前 {}）≥{}；{}",
                            name, layer, min_layer, sub_text
                        ),
                        status: if *layer >= min_layer && ok_sub {
                            "done"
                        } else {
                            "todo"
                        }
            ...
            "techniques_count_min_layer" => {
                ...
                out.push(RealmRequirementView {
                    id: if id.is_empty() {
                        format!("techcnt-{min_count}-{min_layer}")
                    } else {
                        id
                    },
                    title,
                    detail: format!(
                        "至少 {} 门功法 ≥ {} 层（当前 {}）",
                        min_count, min_layer, count
                    ),
                    status: if count >= min_count { "done" } else { "todo" }.to_string(),
                    source_type: None,
                    source_ref: None,
                });
            }
            "item_qty_min" => {
                ...
                let qty_have = if item_def_id.is_empty() {
                    0
                } else {
                    get_item_qty_in_bag(state, character_id, &item_def_id).await?
                };
                let item_name = item_map
                    .get(&item_def_id)
                ...
                out.push(RealmRequirementView {
                    id: if id.is_empty() {
                        format!("item-{item_def_id}")
                    } else {
                        id
                    },
                    title,
                    detail: format!("{} × {}（当前 {}）", item_name, qty_need, qty_have),
                    status: if qty_have >= qty_need { "done" } else { "todo" }.to_string(),
                    source_type: None,
                    source_ref: None,
                });
            }
            "dungeon_clear_min" => {
                ...
                let clear_count = get_dungeon_clear_count(
                    state,
                    character_id,
                    ...
                    if difficulty_id.is_empty() {
                        ...
                        Some(difficulty_id.as_str())
                    },
                )
                .await?;
                let dungeon_name = if dungeon_id.is_empty() {
                    String::new()
                } else {
                    dungeon_map
                        .get(&dungeon_id)
                        .cloned()
                        .unwrap_or_else(|| "目标秘境".to_string())
                };
                let difficulty_name = if difficulty_id.is_empty() {
                    String::new()
                } else {
                    dungeon_map
                        .get(&difficulty_id)
                        .cloned()
                        .unwrap_or_else(|| "指定难度".to_string())
                };
                let scope_text = if !dungeon_id.is_empty() {
                    if !difficulty_id.is_empty() {
                        format!("{}（{}）", dungeon_name, difficulty_name)
                    } else {
                        dungeon_name
                    }
                } else if !difficulty_id.is_empty() {
                ...
                out.push(RealmRequirementView {
                    ...
                    detail: format!(
                        "{} 通关 ≥ {} 次（当前 {}）",
                        scope_text, min_count, clear_count
                    ),
                    ...
                    source_ref: Some(if !difficulty_id.is_empty() {
                        format!(
                            "dungeon:{}|difficulty:{}",
                            if dungeon_id.is_empty() {
                                "*"
                    ...
                    } else if !dungeon_id.is_empty() {
                        format!("dungeon:{}", dungeon_id)
                    } else {
                        "dungeon:*".to_string()
                    }),
                });
            ...
            "main_quest_chapter_completed" => {
                ...
                let completed = if let Some(cache) = &completed_chapters_cache {
                    cache.clone()
                } else {
                    let loaded = get_completed_main_quest_chapter_set(state, character_id).await?;
                    completed_chapters_cache = Some(loaded.clone());
                    loaded
                };
                let done = !chapter_id.is_empty() && completed.contains(&chapter_id);
                let chapter_name = chapter_map
                    .get(&chapter_id)
                    .cloned()
                ...
                out.push(RealmRequirementView {
                    ...
                    source_ref: Some(if chapter_id.is_empty() {
                        "chapter:*".to_string()
                    } else {
    ...
    for cost in costs {
        match cost {
            BreakthroughCost::Exp { amount } if amount > 0 => {
                views.push(RealmCostView {
                    id: "cost-exp".to_string(),
                    title: "经验".to_string(),
                    detail: format!("需要 {}（当前 {}）", amount, current_exp),
            ...
            BreakthroughCost::SpiritStones { amount } if amount > 0 => {
                views.push(RealmCostView {
                    ...
                    title: "灵石".to_string(),
                    detail: format!("需要 {}（当前 {}）", amount, current_spirit_stones),
                    cost_type: "spirit_stones".to_string(),
                    status: Some(
                        if current_spirit_stones >= amount {
                            "done"
                        } else {
                            "todo"
                        }
            ...
            BreakthroughCost::Items { items } => {
                for item in items
                    .into_iter()
                    .filter(|item| item.qty > 0 && !item.item_def_id.trim().is_empty())
                {
                    let have = get_item_qty_in_bag(state, character_id, &item.item_def_id).await?;
                    let meta = item_map.get(&item.item_def_id);
                    views.push(RealmCostView {
                        id: format!("cost-item-{}", item.item_def_id),
                        title: meta
                            .map(|value| value.0.clone())
                            .unwrap_or_else(|| item.item_def_id.clone()),
                        detail: format!("×{}（当前 {}）", item.qty, have),
                        cost_type: "item".to_string(),
                        status: Some(if have >= item.qty { "done" } else { "todo" }.to_string()),
                        amount: None,
                        item_def_id: Some(item.item_def_id.clone()),
                        item_name: meta.map(|value| value.0.clone()),
                        item_icon: meta.and_then(|value| value.1.clone()),
                        qty: Some(item.qty),
                    });
                }
            }
    ...
    for (key, title) in [
        ...
        if flat != 0.0 {
            out.push(RealmRewardView {
                ...
                detail: format!("{}{:.0}", if flat > 0.0 { "+" } else { "" }, flat),
            });
        }
        let pct = rewards
            .pct
        ...
        if pct != 0.0 {
            out.push(RealmRewardView {
                ...
                detail: format!(
                    "{}{}%",
                    if pct > 0.0 { "+" } else { "" },
                    trim_trailing_zeros(pct * 100.0)
                ),
            });
        }
    }
    let control_res = rewards
        .add_percent
        .as_ref()
        .and_then(|map| map.get("kongzhi_kangxing"))
        .copied()
        .unwrap_or_default();
    if control_res != 0.0 {
        out.push(RealmRewardView {
            id: "add-percent-kongzhi_kangxing".to_string(),
            title: "控制抗性".to_string(),
            detail: format!(
                "{}{}%",
                if control_res > 0.0 { "+" } else { "" },
                trim_trailing_zeros(control_res * 100.0)
            ),
        });
    }
    out
...
fn load_realm_breakthrough_config()
-> Result<(RealmBreakthroughConfigFile, Option<String>), AppError> {
    let candidates = [
        std::env::var("REALM_CONFIG_PATH")
            .ok()
            .filter(|value| !value.trim().is_empty()),
        Some(format!(
            "{}/../server/src/data/seeds/realm_breakthrough.json",
            env!("CARGO_MANIFEST_DIR")
        )),
        Some(format!(
            "{}/data/seeds/realm_breakthrough.json",
            env!("CARGO_MANIFEST_DIR")
        )),
    ];
    for candidate in candidates.into_iter().flatten() {
        ...
        let content = fs::read_to_string(&path).map_err(|error| {
            AppError::config(format!("failed to read realm_breakthrough.json: {error}"))
        })?;
        let config: RealmBreakthroughConfigFile =
            serde_json::from_str(&content).map_err(|error| {
                AppError::config(format!("failed to parse realm_breakthrough.json: {error}"))
            })?;
...
fn load_technique_name_map() -> Result<BTreeMap<String, String>, AppError> {
    ...
    let file: TechniqueDefFile = serde_json::from_str(&content).map_err(|error| {
        AppError::config(format!("failed to parse technique_def.json: {error}"))
    })?;
    Ok(file
        .techniques
        .into_iter()
        .filter(|technique| technique.enabled != Some(false))
        .map(|technique| (technique.id, technique.name))
        .collect())
}
...
fn load_dungeon_name_map() -> Result<BTreeMap<String, String>, AppError> {
    ...
    for entry in fs::read_dir(seed_dir).map_err(|error| {
        AppError::config(format!("failed to read dungeon seed directory: {error}"))
    })? {
        let entry = entry.map_err(|error| {
            AppError::config(format!("failed to iterate dungeon seed directory: {error}"))
        })?;
        let file_name = entry.file_name().to_string_lossy().to_string();
        if !file_name.starts_with("dungeon_") || !file_name.ends_with(".json") {
            continue;
        }
        let content = fs::read_to_string(entry.path())
            .map_err(|error| AppError::config(format!("failed to read {file_name}: {error}")))?;
        let file: DungeonSeedFile = serde_json::from_str(&content)
            .map_err(|error| AppError::config(format!("failed to parse {file_name}: {error}")))?;
        for dungeon in file.dungeons {
            if dungeon.def.enabled != Some(false) {
                map.insert(dungeon.def.id.clone(), dungeon.def.name.clone());
            }
            for difficulty in dungeon.difficulties {
                if difficulty.enabled != Some(false) {
                    map.insert(difficulty.id.clone(), difficulty.name.clone());
                }
            }
        }
    }
...
fn load_main_quest_chapter_map() -> Result<BTreeMap<String, String>, AppError> {
    ...
    let seed_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../server/src/data/seeds");
    for entry in fs::read_dir(seed_dir).map_err(|error| {
        AppError::config(format!("failed to read main quest seed directory: {error}"))
    })? {
        let entry = entry.map_err(|error| {
            AppError::config(format!(
                "failed to iterate main quest seed directory: {error}"
            ))
        })?;
        let file_name = entry.file_name().to_string_lossy().to_string();
        ...
        let content = fs::read_to_string(entry.path())
            .map_err(|error| AppError::config(format!("failed to read {file_name}: {error}")))?;
        let file: MainQuestChapterFile = serde_json::from_str(&content)
            .map_err(|error| AppError::config(format!("failed to parse {file_name}: {error}")))?;
        for chapter in file
            .chapters
            .into_iter()
...
async fn get_equipped_main_technique(
    state: &AppState,
    character_id: i64,
    ...
    let technique_id: String = row
        .try_get::<Option<String>, _>("technique_id")?
        .unwrap_or_default();
    if technique_id.trim().is_empty() {
        return Ok(None);
    }
    let name = technique_map
        .get(technique_id.trim())
        .cloned()
        .unwrap_or_else(|| technique_id.trim().to_string());
    let layer = opt_i64_from_i32(&row, "current_layer")?.unwrap_or_default();
    Ok(Some((name, layer)))
}

async fn get_equipped_sub_techniques(
    state: &AppState,
    character_id: i64,
    ...
    for row in rows {
        ...
        if technique_id.trim().is_empty() || slot_index <= 0 {
            continue;
        }
        let name = technique_map
            .get(technique_id.trim())
...
async fn get_technique_count_min_layer(
    state: &AppState,
    character_id: i64,
    min_layer: i64,
) -> Result<i64, AppError> {
...
async fn get_dungeon_clear_count(
    state: &AppState,
    character_id: i64,
    ...
    let mut sql = String::from(
        "SELECT COUNT(1)::bigint AS cnt FROM dungeon_record WHERE character_id = $1 AND result = 'cleared'",
    );
    let rows = state
        .database
        .fetch_one(
            if dungeon_id.is_none() && difficulty_id.is_none() {
                ...
                return state
                    .database
                    .fetch_one(&sql, |query| query.bind(character_id).bind(dungeon_id))
                    .await
                    .and_then(|row| Ok(row.try_get::<Option<i64>, _>("cnt")?.unwrap_or_default()));
            } else {
            ...
            |query| {
                if let Some(difficulty_id) = difficulty_id {
                    query.bind(character_id).bind(difficulty_id)
                } else {
...
async fn get_completed_main_quest_chapter_set(
    state: &AppState,
    character_id: i64,
...
mod tests {
    ...
    fn rewards_view_formats_breakthrough_rewards() {
        let rewards = super::build_rewards_view(Some(super::BreakthroughRewards {
            ...
            flat: Some(std::collections::BTreeMap::from([(
                "max_qixue".to_string(),
                50.0,
            )])),
            pct: Some(std::collections::BTreeMap::from([(
                "wugong".to_string(),
                0.05,
            )])),
            add_percent: Some(std::collections::BTreeMap::from([(
                "kongzhi_kangxing".to_string(),
                0.03,
            )])),
        }));
        ...
        assert!(
            rewards
                .iter()
                .any(|row| row.id == "ap" && row.detail == "+5")
        );
        assert!(
            rewards
                .iter()
                .any(|row| row.id == "flat-max_qixue" && row.detail == "+50")
        );
        assert!(
            rewards
                .iter()
                .any(|row| row.id == "pct-wugong" && row.detail == "+5%")
        );
        assert!(
            rewards
                .iter()
                .any(|row| row.id == "add-percent-kongzhi_kangxing" && row.detail == "+3%")
        );
    }
    ...
    fn realm_config_loads_current_seed_order() {
        let (config, path) =
            super::load_realm_breakthrough_config().expect("realm config should load");
        assert!(path.is_some());
        assert_eq!(config.realm_order.first().map(String::as_str), Some("凡人"));
        assert_eq!(
            config.realm_order.get(1).map(String::as_str),
            Some("炼精化炁·养气期")
        );
    ...
    fn breakthrough_success_payload_matches_frontend_contract_shape() {
        let result = super::success_result(
            ...
            super::RealmBreakthroughData {
                ...
                debug_realtime: Some(build_realm_update_payload(
                    "breakthrough",
                    "凡人",
                    "炼精化炁·养气期",
                )),
                debug_rank_realtime: Some(crate::realtime::rank::build_rank_update_payload(
                    "breakthrough",
                    &["realm", "power"],
                )),
            },
