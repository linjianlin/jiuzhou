...
pub async fn get_enabled_techniques(
    ...
    techniques.sort_by(|left, right| {
        right.sort_weight.cmp(&left.sort_weight)
...
pub async fn get_technique_detail(
    ...
    let detail = load_technique_detail_data(&state, technique_id.trim(), viewer_character_id, false).await?
...
pub(crate) async fn load_technique_detail_data(
    ...
    let seed_technique = techniques.into_iter().find(|technique| technique.id == technique_id);
    ...
    if !allow_partner_only {
        let visibility = if can_character_view_technique_sensitive_layers(state, technique.id.as_str(), viewer_character_id).await? {
    ...
    Ok(Some(TechniqueDetailData { technique, layers, skills }))
}
    ...
    Ok(load_technique_layers()?
        ...
        .map(|layer| TechniqueLayerDto {
            ...
            cost_materials: layer
                ...
                .into_iter()
                .map(|cost| {
                    let meta = item_meta.get(cost.item_id.trim()).cloned().unwrap_or((cost.item_id.clone(), None));
...
fn load_seed_skill_defs(technique_id: &str) -> Result<Vec<SkillDefDto>, AppError> {
    Ok(load_skill_defs()?
        ...
        .filter(|skill| skill.source_type == "technique" && skill.source_id.as_deref() == Some(technique_id))
        .collect())
}
...
fn apply_technique_layer_visibility(layers: Vec<TechniqueLayerDto>, visibility: TechniqueLayerVisibility) -> Vec<TechniqueLayerDto> {
...
async fn load_viewer_character_id(state: &AppState, headers: &HeaderMap) -> Result<Option<i64>, AppError> {
...
async fn load_generated_technique_layers(
    ...
    rows.into_iter().map(|row| {
        let materials = row
            ...
            .into_iter()
            .filter_map(|value| {
                let item_id = value.get("itemId").and_then(|v| v.as_str()).or_else(|| value.get("item_id").and_then(|v| v.as_str()))?.to_string();
                let qty = value.get("qty").and_then(|v| v.as_i64())?;
                let meta = item_meta.get(item_id.trim()).cloned().unwrap_or((item_id.clone(), None));
        ...
        let passives = row
            ...
            .unwrap_or_default();
        let unlock_skill_ids = row.try_get::<Vec<String>, _>("unlock_skill_ids").unwrap_or_default();
        let upgrade_skill_ids = row.try_get::<Vec<String>, _>("upgrade_skill_ids").unwrap_or_default();
        Ok(TechniqueLayerDto {
            technique_id: technique_id.to_string(),
            layer: row.try_get::<Option<i32>, _>("layer")?.map(i64::from).unwrap_or(1),
            cost_spirit_stones: scale_technique_base_cost_by_quality(row.try_get::<Option<i32>, _>("cost_spirit_stones")?.map(i64::from).unwrap_or_default(), quality_multiplier),
            cost_exp: scale_technique_base_cost_by_quality(row.try_get::<Option<i32>, _>("cost_exp")?.map(i64::from).unwrap_or_default(), quality_multiplier),
...
async fn load_generated_skill_defs(state: &AppState, technique_id: &str) -> Result<Vec<SkillDefDto>, AppError> {
    let rows = state.database.fetch_all(
    ...
    rows.into_iter().map(|row| {
        let effects = row.try_get::<Option<serde_json::Value>, _>("effects")?
            .and_then(|value| value.as_array().cloned())
            .unwrap_or_default();
        ...
        source_type: row.try_get::<Option<String>, _>("source_type")?.unwrap_or_else(|| "technique".to_string()),
        source_id: row.try_get::<Option<String>, _>("source_id")?,
        cost_lingqi: row.try_get::<Option<i32>, _>("cost_lingqi")?.map(i64::from).unwrap_or_default(),
        cost_lingqi_rate: row.try_get::<Option<f64>, _>("cost_lingqi_rate")?.unwrap_or_default(),
        cost_qixue: row.try_get::<Option<i32>, _>("cost_qixue")?.map(i64::from).unwrap_or_default(),
        cost_qixue_rate: row.try_get::<Option<f64>, _>("cost_qixue_rate")?.unwrap_or_default(),
        cooldown: normalize_generated_technique_skill_cooldown(
            row.try_get::<Option<i32>, _>("cooldown")?.map(i64::from),
            trigger_type.as_str(),
        ),
        target_type: row.try_get::<Option<String>, _>("target_type")?.unwrap_or_else(|| "single_enemy".to_string()),
        target_count: row.try_get::<Option<i32>, _>("target_count")?.map(i64::from).unwrap_or(1),
        damage_type: row.try_get::<Option<String>, _>("damage_type")?,
        element: row.try_get::<Option<String>, _>("element")?.unwrap_or_else(|| "none".to_string()),
        effects,
        trigger_type,
        conditions: row.try_get::<Option<serde_json::Value>, _>("conditions")?,
        ai_priority: row.try_get::<Option<i32>, _>("ai_priority")?.map(i64::from).unwrap_or(50),
        ai_conditions: row.try_get::<Option<serde_json::Value>, _>("ai_conditions")?,
        upgrades: Some(row.try_get::<Option<serde_json::Value>, _>("upgrades")?.and_then(|value| value.as_array().cloned()).unwrap_or_default()),
        sort_weight: row.try_get::<Option<i32>, _>("sort_weight")?.map(i64::from).unwrap_or_default(),
        version: row.try_get::<Option<i32>, _>("version")?.map(i64::from).unwrap_or(1),
        enabled: row.try_get::<Option<bool>, _>("enabled")?.unwrap_or(true),
    })}).collect()
}

pub(crate) fn resolve_skill_trigger_type(trigger_type: Option<&str>, effects: &[serde_json::Value]) -> String {
    let has_aura_effect = effects.iter().any(|effect| {
        let effect_type = effect.get("type").and_then(|value| value.as_str()).map(str::trim).unwrap_or("");
    ...
    match trigger_type.map(str::trim) {
        Some("active" | "passive" | "counter" | "chase") => trigger_type.unwrap().trim().to_string(),
        _ => "active".to_string(),
    }
}

fn normalize_generated_technique_skill_cooldown(raw_cooldown: Option<i64>, trigger_type: &str) -> i64 {
...
async fn load_generated_technique_def(state: &AppState, technique_id: &str, allow_partner_only: bool) -> Result<Option<TechniqueDefDto>, AppError> {
    let row = state.database.fetch_optional(
    ...
    if !allow_partner_only && normalize_technique_usage_scope(row.try_get::<Option<String>, _>("usage_scope")?.as_deref()) == "partner_only" {
        return Ok(None);
    }
    let quality = row.try_get::<Option<String>, _>("quality")?.unwrap_or_else(|| "黄".to_string());
    let tags = row
    ...
    Ok(Some(TechniqueDefDto {
        ...
        code: None,
        name: row.try_get::<Option<String>, _>("display_name")?.unwrap_or_else(|| technique_id.to_string()),
        technique_type: row.try_get::<Option<String>, _>("type")?.unwrap_or_else(|| "功法".to_string()),
        quality_rank: quality_rank_from_name(Some(quality.as_str())),
        quality,
        max_layer: row.try_get::<Option<i32>, _>("max_layer")?.map(i64::from).unwrap_or_default(),
        required_realm: row.try_get::<Option<String>, _>("required_realm")?.unwrap_or_else(|| "凡人".to_string()),
...
async fn load_generated_technique_cost_multiplier(state: &AppState, technique_id: &str) -> Result<i64, AppError> {
    let row = state.database.fetch_optional(
...
fn load_technique_defs() -> Result<Vec<TechniqueDefDto>, AppError> {
    ...
    Ok(payload.techniques.into_iter().filter(|row| row.enabled != Some(false)).map(|row| {
...
async fn load_generated_visible_technique_defs(state: &AppState) -> Result<Vec<TechniqueDefDto>, AppError> {
    let rows = state.database.fetch_all(
    ...
    rows.into_iter().map(|row| {
        ...
        Ok(TechniqueDefDto {
            ...
            code: None,
            name: row.try_get::<Option<String>, _>("display_name")?.unwrap_or_default(),
            technique_type: row.try_get::<Option<String>, _>("type")?.unwrap_or_else(|| "功法".to_string()),
            quality_rank: quality_rank_from_name(Some(quality.as_str())),
            quality,
            max_layer: row.try_get::<Option<i32>, _>("max_layer")?.map(i64::from).unwrap_or(1),
            required_realm: row.try_get::<Option<String>, _>("required_realm")?.unwrap_or_else(|| "凡人".to_string()),
            attribute_type: row.try_get::<Option<String>, _>("attribute_type")?.unwrap_or_else(|| "physical".to_string()),
            attribute_element: row.try_get::<Option<String>, _>("attribute_element")?.unwrap_or_else(|| "none".to_string()),
            tags,
...
fn load_technique_layers() -> Result<Vec<RawTechniqueLayer>, AppError> {
    ...
    let payload: TechniqueLayerFile = serde_json::from_str(&normalized_content)
        .map_err(|error| AppError::config(format!("failed to parse technique_layer.json [technique.rs]: {error}")))?;
    Ok(payload.layers)
}
...
fn load_skill_defs() -> Result<Vec<SkillDefDto>, AppError> {
    ...
    Ok(payload.skills.into_iter().map(|row| SkillDefDto {
        id: row.id,
        code: row.code,
        name: row.name,
        description: row.description,
        ...
        target_type: row.target_type.unwrap_or_else(|| "single_enemy".to_string()),
...
mod tests {
    ...
    fn technique_detail_payload_matches_contract() {
        ...
        assert_eq!(payload["data"]["layers"][0]["technique_id"], "tech-yangqi-jue");
