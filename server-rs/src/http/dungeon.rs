...
use crate::jobs::online_battle_settlement::{DungeonClearSettlementTaskPayload, enqueue_dungeon_clear_settlement_task};
...
pub async fn get_dungeon_categories() -> Result<Json<SuccessResponse<serde_json::Value>>, AppError> {
    ...
    let categories = ordered
        ...
        .collect::<Vec<_>>();
    Ok(send_success(serde_json::json!({ "categories": categories })))
}
...
pub async fn get_dungeon_list(
    ...
    let realm = query.realm.as_deref().unwrap_or_default().trim().to_string();
    let dungeon_type = query.r#type.as_deref().unwrap_or_default().trim().to_string();
    ...
    dungeons.sort_by(|left, right| right.sort_weight.cmp(&left.sort_weight).then_with(|| left.id.cmp(&right.id)));
...
pub async fn get_dungeon_preview(
    ...
    let difficulty = entry
        .difficulties
        .iter()
        .find(|row| row.get("difficulty_rank").and_then(|value| value.as_i64()).unwrap_or(1) == rank)
        .cloned();
    ...
    let stages_value = difficulty.get("stages").and_then(|value| value.as_array()).cloned().unwrap_or_default();
            ...
            let waves = stage
                ...
                .map(|wave| {
                    let monsters = wave
                        ...
                        .into_iter()
                        .map(|monster| {
                            let monster_id = monster.get("monster_def_id").and_then(|value| value.as_str()).unwrap_or_default().to_string();
                            let meta = monster_map.get(&monster_id).cloned().unwrap_or_else(|| serde_json::json!({}));
                            if let Some(rewards) = difficulty.get("first_clear_rewards").and_then(|v| v.get("items")).and_then(|v| v.as_array()) {
                                for reward in rewards {
                                    if let Some(item_id) = reward.get("item_def_id").and_then(|value| value.as_str()) {
                                        if let Some(item_def) = item_map.get(item_id) {
                                            drop_item_ids.entry(item_id.to_string()).or_insert(DungeonPreviewDropItemDto {
                                                id: item_id.to_string(),
                                                name: item_def.get("name").and_then(|value| value.as_str()).unwrap_or(item_id).to_string(),
                                                quality: item_def.get("quality").and_then(|value| value.as_str()).map(|value| value.to_string()),
                                            });
                                        }
                                    }
                                }
                            ...
                            DungeonPreviewMonsterDto {
                                id: monster_id.clone(),
                                name: meta.get("name").and_then(|value| value.as_str()).unwrap_or(&monster_id).to_string(),
                                realm: meta.get("realm").and_then(|value| value.as_str()).map(|value| value.to_string()),
                                level: meta.get("level").and_then(|value| value.as_i64()).unwrap_or_default(),
                                avatar: meta.get("avatar").and_then(|value| value.as_str()).map(|value| value.to_string()),
                                kind: meta.get("kind").and_then(|value| value.as_str()).map(|value| value.to_string()),
                                count: monster.get("count").and_then(|value| value.as_i64()).unwrap_or(1),
                                drop_pool_id: meta.get("drop_pool_id").and_then(|value| value.as_str()).map(|value| value.to_string()),
                                drop_preview: Vec::new(),
                            }
                        })
                        .collect::<Vec<_>>();
                    DungeonPreviewWaveDto {
                        wave_index: wave.get("wave_index").and_then(|value| value.as_i64()).unwrap_or(1),
                        spawn_delay_sec: wave.get("spawn_delay_sec").and_then(|value| value.as_i64()).unwrap_or_default(),
                        monsters,
                    }
                })
                .collect::<Vec<_>>();
            DungeonPreviewStageDto {
                id: stage.get("id").and_then(|value| value.as_str()).unwrap_or_default().to_string(),
                stage_index: stage.get("stage_index").and_then(|value| value.as_i64()).unwrap_or(1),
                name: stage.get("name").and_then(|value| value.as_str()).map(|value| value.to_string()),
                r#type: stage.get("type").and_then(|value| value.as_str()).unwrap_or("battle").to_string(),
                waves,
            }
        })
        .collect::<Vec<_>>();
    let difficulty_dto = DungeonDifficultyDto {
        id: difficulty.get("id").and_then(|value| value.as_str()).unwrap_or_default().to_string(),
        name: difficulty.get("name").and_then(|value| value.as_str()).unwrap_or_default().to_string(),
        difficulty_rank: difficulty.get("difficulty_rank").and_then(|value| value.as_i64()).unwrap_or(1),
    };
...
pub async fn create_dungeon_instance(
    ...
    let result = state.database.with_transaction(|| async {
        create_dungeon_instance_tx(&state, actor.user_id, dungeon_id.trim(), difficulty_rank).await
    }).await?;
...
pub async fn get_dungeon_instance_by_battle_id(
    ...
    let actor = auth::require_auth(&state, &headers).await?;
    let result = get_dungeon_instance_by_battle_id_tx(&state, actor.user_id, battle_id.trim()).await?;
    Ok(send_result(result))
}
...
fn load_all_dungeon_defs() -> Result<Vec<DungeonDefDto>, AppError> {
    ...
    for path in glob_dungeon_seed_paths()? {
        ...
        let payload: DungeonSeedFile = serde_json::from_str(&content).map_err(|error| AppError::config(format!("failed to parse {}: {error}", path.display())))?;
...
pub(crate) fn load_dungeon_wave_monster_ids(
    ...
    let stages = difficulty.get("stages").and_then(|v| v.as_array()).cloned().unwrap_or_default();
    let monster_ids = stages
        ...
        .and_then(|waves| waves.iter().find(|wave| wave.get("wave_index").and_then(|v| v.as_i64()) == Some(wave_index)))
        .map(build_wave_monster_ids)
        .unwrap_or_default();
    if monster_ids.is_empty() {
        return Ok(vec![format!("dungeon-monster-{}-{}", stage_index.max(1), wave_index.max(1))]);
    }
    Ok(monster_ids)
}
...
fn map_dungeon_def(value: &serde_json::Value) -> Result<DungeonDefDto, AppError> {
    ...
    Ok(DungeonDefDto {
        ...
        category: value.get("category").and_then(|v| v.as_str()).map(|v| v.to_string()),
        description: value.get("description").and_then(|v| v.as_str()).map(|v| v.to_string()),
        icon: value.get("icon").and_then(|v| v.as_str()).map(|v| v.to_string()),
        background: value.get("background").and_then(|v| v.as_str()).map(|v| v.to_string()),
        min_players: value.get("min_players").and_then(|v| v.as_i64()).unwrap_or(1),
        max_players: value.get("max_players").and_then(|v| v.as_i64()).unwrap_or(5),
        min_realm: value.get("min_realm").and_then(|v| v.as_str()).map(|v| v.to_string()),
        recommended_realm: value.get("recommended_realm").and_then(|v| v.as_str()).map(|v| v.to_string()),
        unlock_condition: value.get("unlock_condition").cloned().unwrap_or_else(|| serde_json::json!({})),
        daily_limit: value.get("daily_limit").and_then(|v| v.as_i64()).unwrap_or_default(),
        weekly_limit: value.get("weekly_limit").and_then(|v| v.as_i64()).unwrap_or_default(),
        stamina_cost: value.get("stamina_cost").and_then(|v| v.as_i64()).unwrap_or_default(),
        time_limit_sec: value.get("time_limit_sec").and_then(|v| v.as_i64()).unwrap_or_default(),
        revive_limit: value.get("revive_limit").and_then(|v| v.as_i64()).unwrap_or_default(),
        tags: value.get("tags").cloned().unwrap_or_else(|| serde_json::json!([])),
        sort_weight: value.get("sort_weight").and_then(|v| v.as_i64()).unwrap_or_default(),
...
fn load_monster_seed_map() -> Result<BTreeMap<String, serde_json::Value>, AppError> {
    ...
    Ok(payload.monsters.into_iter().filter_map(|row| row.get("id").and_then(|v| v.as_str()).map(|id| (id.to_string(), row.clone()))).collect())
}
...
fn load_item_seed_map() -> Result<BTreeMap<String, serde_json::Value>, AppError> {
    ...
    for filename in ["item_def.json", "equipment_def.json", "gem_def.json"] {
        ...
        let content = fs::read_to_string(&path).map_err(|error| AppError::config(format!("failed to read {}: {error}", path.display())))?;
        let payload: ItemSeedFile = serde_json::from_str(&content).map_err(|error| AppError::config(format!("failed to parse {}: {error}", path.display())))?;
...
fn realm_rank(realm: &str) -> i64 {
    ...
    ORDER.iter().position(|item| *item == realm.trim()).map(|idx| idx as i64 + 1).unwrap_or(0)
}
...
async fn create_dungeon_instance_tx(
    ...
    let Some(character_row) = state.database.fetch_optional(
    ...
    let Some(entry) = load_dungeon_entry(dungeon_id)? else {
        return Ok(ServiceResult { success: false, message: Some("秘境不存在".to_string()), data: None });
    };
    let dungeon = map_dungeon_def(&entry.def)?;
    let difficulty = entry
        .difficulties
        .iter()
        .find(|row| row.get("difficulty_rank").and_then(|value| value.as_i64()).unwrap_or(1) == difficulty_rank)
        .cloned();
    let Some(difficulty) = difficulty else {
        return Ok(ServiceResult { success: false, message: Some("秘境难度不存在".to_string()), data: None });
    };
    ...
    let participants = if team_members.is_empty() {
        vec![DungeonInstanceParticipantDto { user_id, character_id, role: "leader".to_string() }]
    } else {
        team_id = team_members.first().and_then(|row| row.try_get::<Option<String>, _>("team_id").ok().flatten());
        let is_leader = team_members.iter().any(|row| row.try_get::<Option<i32>, _>("character_id").ok().flatten().map(i64::from) == Some(character_id) && row.try_get::<Option<String>, _>("role").ok().flatten().as_deref() == Some("leader"));
        if !is_leader {
            return Ok(ServiceResult { success: false, message: Some("组队中只有队长可以创建秘境".to_string()), data: None });
        }
        team_members
            .into_iter()
            .filter_map(|row| {
        let user_id = row.try_get::<Option<i32>, _>("user_id").ok().flatten().map(i64::from)?;
        let character_id = row.try_get::<Option<i32>, _>("character_id").ok().flatten().map(i64::from)?;
                let role = row.try_get::<Option<String>, _>("role").ok().flatten().unwrap_or_else(|| "member".to_string());
                Some(DungeonInstanceParticipantDto { user_id, character_id, role })
            })
            .collect::<Vec<_>>()
    };

    if participants.len() < dungeon.min_players.max(1) as usize {
        return Ok(ServiceResult { success: false, message: Some(format!("人数不足，需要至少{}人", dungeon.min_players)), data: None });
    }
    if participants.len() > dungeon.max_players.max(1) as usize {
        return Ok(ServiceResult { success: false, message: Some(format!("人数超限，最多{}人", dungeon.max_players)), data: None });
    }
    if let Some(min_realm) = dungeon.min_realm.as_deref()
        && realm_rank(&full_realm) < realm_rank(min_realm)
    {
        return Ok(ServiceResult { success: false, message: Some(format!("境界不足，需要{}以上", min_realm)), data: None });
    }
    ...
    let difficulty_id = difficulty.get("id").and_then(|value| value.as_str()).unwrap_or_default().to_string();
...
async fn get_dungeon_instance_tx(
    ...
    let Some(row) = row else {
        return Ok(ServiceResult { success: false, message: Some("秘境实例不存在".to_string()), data: None });
    };
    let snapshot = build_dungeon_instance_snapshot(&row)?;
    if !snapshot.participants.iter().any(|participant| participant.user_id == user_id) {
        return Ok(ServiceResult { success: false, message: Some("无权访问该秘境".to_string()), data: None });
    }
    Ok(ServiceResult { success: true, message: None, data: Some(DungeonInstanceQueryDataDto { instance: snapshot }) })
}
...
async fn join_dungeon_instance_tx(
    ...
    if snapshot.status != "preparing" {
        return Ok(ServiceResult { success: false, message: Some("该秘境已开始或已结束".to_string()), data: None });
    }
    if snapshot.participants.iter().any(|participant| participant.user_id == user_id) {
        return Ok(ServiceResult { success: true, message: None, data: Some(CreateDungeonInstanceDataDto { instance_id: snapshot.id, status: snapshot.status, participants: snapshot.participants }) });
    }
    ...
    if instance_team_id.is_none() || participant_team_id != instance_team_id {
        return Ok(ServiceResult { success: false, message: Some("不是同一队伍，无法加入".to_string()), data: None });
    }
    ...
    let participants = team_rows
        ...
        .filter_map(|row| {
            ...
            user_id: row.try_get::<Option<i32>, _>("user_id").ok().flatten().map(i64::from)?,
            character_id: row.try_get::<Option<i32>, _>("character_id").ok().flatten().map(i64::from)?,
                role: row.try_get::<Option<String>, _>("role").ok().flatten().unwrap_or_else(|| "member".to_string()),
            })
        })
        .collect::<Vec<_>>();
    ...
    let realm = character_row.try_get::<Option<String>, _>("realm")?.unwrap_or_else(|| "凡人".to_string());
    let sub_realm = character_row.try_get::<Option<String>, _>("sub_realm")?.unwrap_or_default();
    let full_realm = if realm.trim() == "凡人" || sub_realm.trim().is_empty() { realm } else { format!("{}·{}", realm.trim(), sub_realm.trim()) };
    let difficulty_min_realm = entry.difficulties.iter().find(|diff| diff.get("id").and_then(|v| v.as_str()) == Some(snapshot.difficulty_id.as_str())).and_then(|diff| diff.get("min_realm").and_then(|v| v.as_str())).map(|v| v.to_string());
    ...
    state.database.execute(
        ...
        |q| q.bind(instance_id).bind(serde_json::to_value(&participants).unwrap_or_else(|_| serde_json::json!([]))),
    ).await?;
    snapshot.participants = participants.clone();
    Ok(ServiceResult { success: true, message: None, data: Some(CreateDungeonInstanceDataDto { instance_id: snapshot.id, status: snapshot.status, participants }) })
}
...
async fn start_dungeon_instance_tx(
    ...
    let battle_id = format!("dungeon-battle-{}-{}-{}", instance_id, snapshot.current_stage, snapshot.current_wave);
    let owner_character_id = snapshot.participants.first().map(|participant| participant.character_id).unwrap_or_default();
    ...
    state.online_battle_projections.register(OnlineBattleProjectionRecord {
        ...
        participant_user_ids: snapshot.participants.iter().map(|participant| participant.user_id).collect(),
    ...
    Ok(ServiceResult {
        ...
        data: Some(DungeonInstanceStartDataDto {
            ...
            battle_id,
            state: Some(serde_json::to_value(battle_state).map_err(|error| AppError::config(format!("failed to serialize dungeon start state: {error}")))?),
        }),
    })
}
...
async fn abandon_dungeon_instance_tx(
    ...
    if matches!(snapshot.status.as_str(), "completed" | "failed" | "abandoned") {
        return Ok(ServiceResult { success: false, message: Some("该秘境已结束".to_string()), data: None });
    }
    state.database.execute(
    ...
    let updated_session = state.battle_sessions.update(&format!("dungeon-session-{}", instance_id), |record| {
    ...
    snapshot.end_time = Some(time::OffsetDateTime::now_utc().format(&time::format_description::well_known::Rfc3339).unwrap_or_default());
    Ok(ServiceResult {
        success: true,
...
async fn next_dungeon_instance_tx(
    ...
    if snapshot.status != "running" {
        return Ok(ServiceResult { success: false, message: Some("该秘境当前不可推进".to_string()), data: None });
    }
    let Some(current_battle_id) = snapshot.current_battle_id.clone() else {
        return Ok(ServiceResult { success: false, message: Some("当前秘境没有进行中的战斗".to_string()), data: None });
    };
    let battle_state = state.battle_runtime.get(&current_battle_id).ok_or_else(|| AppError::config("当前战斗状态不存在"))?;
    let Some(result) = battle_state.result.as_deref() else {
        return Ok(ServiceResult { success: false, message: Some("当前战斗尚未结束".to_string()), data: None });
    };
    ...
    let Some(difficulty) = entry.difficulties.iter().find(|diff| diff.get("id").and_then(|v| v.as_str()) == Some(snapshot.difficulty_id.as_str())) else {
        return Ok(ServiceResult { success: false, message: Some("秘境难度不存在".to_string()), data: None });
    };
    let stages = difficulty.get("stages").and_then(|v| v.as_array()).cloned().unwrap_or_default();
    let next_cursor = find_next_dungeon_cursor(&stages, snapshot.current_stage, snapshot.current_wave);

    if let Some((next_stage, next_wave, monster_ids)) = next_cursor {
        let next_battle_id = format!("dungeon-battle-{}-{}-{}", instance_id, next_stage, next_wave);
        let owner_character_id = snapshot.participants.first().map(|participant| participant.character_id).unwrap_or_default();
        let battle_state = build_minimal_pve_battle_state(&next_battle_id, owner_character_id, &monster_ids);
        state.battle_runtime.register(battle_state.clone());
        state.online_battle_projections.register(OnlineBattleProjectionRecord {
        ...
        let _ = state.battle_sessions.update(&format!("dungeon-session-{}", instance_id), |record| {
        ...
        return Ok(ServiceResult {
            ...
            data: Some(DungeonInstanceNextDataDto {
                ...
                state: Some(serde_json::to_value(battle_state).map_err(|error| AppError::config(format!("failed to serialize dungeon battle state: {error}")))?),
                finished: false,
            }),
        });
    }
    ...
    let reward_recipients = snapshot
        ...
        .map(|participant| crate::jobs::online_battle_settlement::DungeonSettlementRewardRecipient {
...
fn find_next_dungeon_cursor(
    ...
    for stage in stages {
        let stage_index = stage.get("stage_index").and_then(|v| v.as_i64()).unwrap_or_default();
        let waves = stage.get("waves").and_then(|v| v.as_array()).cloned().unwrap_or_default();
        for wave in waves {
            let wave_index = wave.get("wave_index").and_then(|v| v.as_i64()).unwrap_or_default();
            if !found_current {
            ...
            let monster_ids = wave
                ...
                .into_iter()
                .flat_map(|monster| {
                    let id = monster.get("monster_def_id").and_then(|v| v.as_str()).unwrap_or_default().to_string();
                    let count = monster.get("count").and_then(|v| v.as_i64()).unwrap_or(1).max(1);
...
fn build_dungeon_instance_snapshot(row: &sqlx::postgres::PgRow) -> Result<DungeonInstanceSnapshotDto, AppError> {
    ...
    Ok(DungeonInstanceSnapshotDto {
        ...
        status: row.try_get::<Option<String>, _>("status")?.unwrap_or_else(|| "preparing".to_string()),
        current_stage: row.try_get::<Option<i32>, _>("current_stage")?.map(i64::from).unwrap_or(1),
        current_wave: row.try_get::<Option<i32>, _>("current_wave")?.map(i64::from).unwrap_or(1),
        participants,
        current_battle_id: instance_data.get("currentBattleId").and_then(|v| v.as_str()).map(|v| v.to_string()),
        start_time: row.try_get::<Option<String>, _>("start_time_text")?,
        end_time: row.try_get::<Option<String>, _>("end_time_text")?,
        time_spent_sec: row.try_get::<Option<i32>, _>("time_spent_sec")?.map(i64::from).unwrap_or_default(),
        total_damage: row.try_get::<Option<i64>, _>("total_damage")?.unwrap_or_default(),
        death_count: row.try_get::<Option<i32>, _>("death_count")?.map(i64::from).unwrap_or_default(),
    })
}

fn extract_difficulty_rank(instance_data: &serde_json::Value) -> i64 {
    instance_data.get("difficultyRank").and_then(|value| value.as_i64()).unwrap_or(1)
}
...
mod tests {
    ...
    fn dungeon_wave_monster_loader_reads_real_seed_monsters() {
        ...
        println!("DUNGEON_WAVE_MONSTER_IDS={}", serde_json::json!(monster_ids));
    }
    ...
    fn dungeon_list_payload_matches_contract() {
        ...
        assert_eq!(payload["data"]["dungeons"][0]["id"], "dungeon-qiqi-wolf-den");
