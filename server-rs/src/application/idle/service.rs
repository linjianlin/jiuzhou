 ...
 *
 * 作用：
 * 1. 做什么：为 `/api/idle/start|status|stop` 提供最小真实会话读写，保持 Node 兼容的 sessionId / conflict / envelope 所需数据。
 * 2. 做什么：复用 PostgreSQL `idle_sessions` 与 Redis `idle:lock:{characterId}`，并把 lock 变化同步回已挂到 AppState 的 runtime idle 服务。
 * 3. 不做什么：不启动挂机执行循环、不推进战斗批次、不实现 history/config/progress 等未纳入本任务的路由。
 *
 * 输入 / 输出：
 * - 输入：characterId、userId、最小 start 请求参数。
 * - 输出：start/status/stop 所需的最小协议结果，其中 session 视图只包含当前前端已使用字段。
 *
 * 数据流 / 状态流：
 ...
 *
 * 复用设计说明：
 * - 把 DB 查询、Redis 锁与 session 视图映射集中在这里，避免路由层重复写 characterId -> active session -> targetMonsterDefId 提取逻辑。
 * - 当前只产出最小 `IdleSessionView`，后续补执行引擎时仍可继续复用同一条 session 查询链，不需要再改 HTTP 合同。
 *
 * 关键边界条件与坑点：
...
impl RustIdleRouteService {
    ...
    pub async fn start_idle_session(
        ...
        self.register_runtime_lock(character_id, &lock_token).await?;
    ...
    async fn remove_runtime_lock(&self, character_id: i64) -> Result<(), BusinessError> {
        let mut runtime_services = self.runtime_services.write().await;
        runtime_services.idle_runtime_service.remove_lock(character_id);
        Ok(())
    }
...
fn map_idle_session_view(row: sqlx::postgres::PgRow) -> Result<IdleSessionView, BusinessError> {
    ...
    Ok(IdleSessionView {
        ...
        total_silver: row.try_get("total_silver").map_err(internal_business_error)?,
        bag_full_flag: row
