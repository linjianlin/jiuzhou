...
test('getInventoryItems(bag) 不应暴露待 flush 奖励生成的临时 overlay', async (t) => {
  t.mock.method(database, 'query', async (sql: string) => {
    if (sql.includes('INSERT INTO inventory')) {
      return { rows: [] };
    }
...
test('getInventoryItems(bag) 不应把待 flush 可堆叠奖励提前合并到真实实例数量', async (t) => {
  t.mock.method(database, 'query', async (sql: string) => {
    if (sql.includes('INSERT INTO inventory')) {
      return { rows: [] };
    }
    ...
    async () => ([
      {
        ...
        obtained_from: 'bag',
        obtained_ref_id: null,
        created_at: new Date('2026-04-08T10:07:00.000Z'),
      },
    ]),
  );
  ...
  assert.deepEqual(
    result.items.map((item) => ({ id: item.id, item_def_id: item.item_def_id, qty: item.qty, location_slot: item.location_slot })),
    [
      { id: 501, item_def_id: 'mat-base-001', qty: 3, location_slot: 0 },
    ],
  );
});
