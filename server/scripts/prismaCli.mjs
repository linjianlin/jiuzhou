 ...
 * 3. 不做什么：不处理数据库业务逻辑，不改写 Prisma 参数，也不兜底吞掉 CLI 错误。
 *
 * 输入/输出：
 ...
 *
 * 数据流/状态流：
 * - package script -> 本脚本扫描 `node_modules/.pnpm` -> 定位 Prisma CLI -> `node <prisma-cli> ...args`。
 *
 * 关键边界条件与坑点：
 * 1. 当前工作区没有暴露 `prisma` 到 `.bin`，所以必须把 CLI 定位逻辑集中到这里，避免每个脚本都硬编码版本路径。
 * 2. 若未来 Prisma 依赖未安装，本脚本应直接失败并暴露原始问题，而不是静默回退到其他实现。
 */
